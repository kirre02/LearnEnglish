<template>
  <div class="familj-page-container" @keydown="handleKeydown" tabindex="0">
    <div class="familj-container">

      <div class="familj-header" v-if="!isResultsMode">
      ...
      <div v-if="!isResultsMode">
        ...
        <div v-else-if="!quizFinished && currentQuestion" class="familj-content">
      ...
      <div v-else-if="isResultsMode" class="results-page">
        <h2 class="results-title">👨‍👩‍👧‍👦 Dina quizresultat - Familj</h2>
...
export default {
  ...
  data() {
    ...
    const initialQuestions = [
      { question: "Vad betyder 'mamma' på engelska?", options: shuffleArray(["mother", "father", "sister", "brother"]), correctAnswer: "mother", hint: "Hon tar hand om barnen", audioText: "mother" },
      { question: "Vad betyder 'pappa' på engelska?", options: shuffleArray(["father", "mother", "grandfather", "uncle"]), correctAnswer: "father", hint: "Han är make till mamma", audioText: "father" },
      { question: "Vad betyder 'syster' på engelska?", options: shuffleArray(["sister", "brother", "mother", "father"]), correctAnswer: "sister", hint: "Hon är en kvinnlig syskon", audioText: "sister" },
      { question: "Vad betyder 'bror' på engelska?", options: shuffleArray(["brother", "sister", "cousin", "grandfather"]), correctAnswer: "brother", hint: "Han är en manlig syskon", audioText: "brother" },
      { question: "Vad betyder 'familj' på engelska?", options: shuffleArray(["family", "parents", "relatives", "cousins"]), correctAnswer: "family", hint: "Mamma, pappa och barn tillsammans", audioText: "family" }
    ];
  ...
  computed: {
    ...
    resultsMessage() {
      const percentage = (this.score / this.questions.length) * 100;
      if (percentage >= 80) return 'Du är en riktig familjexpert!';
      if (percentage >= 60) return 'Du kan många familjeord!';
  ...
  async mounted() {
    ...
    this.isResultsMode = this.$route.path.includes('/familj/results') || this.$route.query.showResults === 'true';
    
    try {
      if (this.isResultsMode) {
        console.log("📊 Laddar quizresultat för familj...");
        await this.loadPastResults();
      } else {
        console.log("👨‍👩‍👧‍👦 Laddar familjefrågor...");
  ...
  try {
    ...
    useFallbackQuestions() {
      ...
      console.log('Använder fallback-frågor för familj');
    },
  ...
  watch: {
    '$route'(to) {
      if (to.path.includes('/familj/results')) {
        console.log("📡 Route changed → reload familj results");
        this.isResultsMode = true;
        this.loadPastResults();
...
<style scoped>
/* Familj-specifik styling med samma layout som Djur */
...
.play-again-btn {
	background: linear-gradient(135deg, #FF9A8B, #FF6A88); /* Familjens tema-färg */
	color: white;
	border: none;
...
.results-page .results-title {
	color: #FF9A8B; /* Familjens tema-färg */
	text-align: center;
	margin-bottom: 15px;
	font-size: 2.2em;
}
...
.results-table thead th {
	background-color: #FF9A8B; /* Familjens tema-färg */
	color: white;
	font-weight: bold;
...
.results-table tbody tr:hover {
	background-color: #fff2f1; /* Lätt rosa hover för Familj-temat */
...
.back-from-results-btn {
	margin-top: 30px;
	background: linear-gradient(135deg, #FF6B6B, #FF8E53); /* Familjens tema-färg (tillbaka-knapp) */
	color: white;
	padding: 12px 25px;
	border-radius: 25px;
	font-weight: bold;

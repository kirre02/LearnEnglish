<template>
  <div class="mat-page-container" @keydown="handleKeydown" tabindex="0">
    <div class="mat-container">

      <div class="mat-header" v-if="!isResultsMode">
      ...
      <div v-if="!isResultsMode">
        ...
        <div v-else-if="!quizFinished && currentQuestion" class="mat-content">
      ...
      <div v-else-if="isResultsMode" class="results-page">
        <h2 class="results-title">🍎 Dina quizresultat - Mat</h2>
...
export default {
  ...
  data() {
    ...
    const initialQuestions = [
      { question: "Vad betyder 'äpple' på engelska?", options: shuffleArray(["apple", "banana", "pear", "orange"]), correctAnswer: "apple", hint: "En frukt som ofta är röd eller grön", audioText: "apple" },
      { question: "Vad betyder 'banan' på engelska?", options: shuffleArray(["banana", "apple", "pear", "plum"]), correctAnswer: "banana", hint: "En gul frukt som apor gillar", audioText: "banana" },
      { question: "Vad betyder 'päron' på engelska?", options: shuffleArray(["pear", "apple", "banana", "orange"]), correctAnswer: "pear", hint: "En frukt som är grön och formad som en droppe", audioText: "pear" },
      { question: "Vad betyder 'apelsin' på engelska?", options: shuffleArray(["orange", "lemon", "apple", "pear"]), correctAnswer: "orange", hint: "En frukt och en färg med samma namn", audioText: "orange" },
      { question: "Vad betyder 'bröd' på engelska?", options: shuffleArray(["bread", "cheese", "milk", "butter"]), correctAnswer: "bread", hint: "Du kan rosta det till frukost", audioText: "bread" }
    ];
  ...
  computed: {
    ...
    resultsMessage() {
      const percentage = (this.score / this.questions.length) * 100;
      if (percentage >= 80) return 'Du är en riktig matexpert!';
      if (percentage >= 60) return 'Du kan mycket om matord!';
      return 'Fortsätt öva, så blir du bättre på matord!';
    },
    isAnswerCorrect() {
      return this.selectedAnswer === this.currentQuestion.correctAnswer;
    },
  ...
  async mounted() {
    ...
    this.isResultsMode = this.$route.path.includes('/mat/results') || this.$route.query.showResults === 'true';
    
    try {
      if (this.isResultsMode) {
        console.log("📊 Laddar quizresultat för mat...");
        await this.loadPastResults();
      } else {
        console.log("🍎 Laddar matfrågor...");
  ...
  methods: {
    ...
    async loadPastResults() {
      ...
      try {
        ...
        console.log("✅ Filtrerade matresultat:", this.results.length);
      } catch (err) {
  ...
  try {
    ...
    async fetchQuizQuestions() {
      this.loading = true;
      try {
        const response = await fetch('http://localhost:9001/api/words/category/mat');
          ...
          if (words.length > 0) {
            ...
            const dbQuestions = dbWords.map(word => {
              ...
              return {
                ...
                hint: word.hint || `Ord från matkategorin`,
    ...
    useFallbackQuestions() {
      ...
      console.log('Använder fallback-frågor för mat');
    },
  ...
  watch: {
    '$route'(to) {
      if (to.path.includes('/mat/results')) {
        console.log("📡 Route changed → reload mat results");
        this.isResultsMode = true;
        this.loadPastResults();
...
<style scoped>
/* Mat-specifik styling med samma layout som Djur */
.mat-page-container {
	min-height: 100vh;
...
.feedback-bubble.incorrect {
	background: linear-gradient(135deg, #FF9A8B, #FF6A88); /* Tema färg */
	color: white;
}
...
.next-btn {
	background: white;
	color: #FF9A8B; /* Ändrad färg för Next-knappen för att matcha quiz-temat */
	border: none;
	border-radius: 20px;
	padding: 12px 25px;
	cursor: pointer;
...
.play-again-btn {
	background: linear-gradient(135deg, #FF9A8B, #FF6A88); /* Mat tema-färg */
	color: white;
	border: none;
...
.results-table thead th {
	background-color: #FF9A8B; /* Mat tema-färg */
	color: white;
	font-weight: bold;
...
.results-table tbody tr:hover {
	background-color: #fff2f1; /* Lätt rosa hover för Mat-temat */
...
.back-from-results-btn {
	margin-top: 30px;
	background: linear-gradient(135deg, #FF9A8B, #FF6A88); /* Mat tema-färg (tillbaka-knapp) */
	color: white;
	padding: 12px 25px;
	border-radius: 25px;
	font-weight: bold;
...
@keyframes pulse {
	0% {
		box-shadow: 0 0 0 0 rgba(255, 154, 139, 0.4);
	}
	70% {
		box-shadow: 0 0 0 10px rgba(255, 154, 139, 0);
	}
	100% {
		box-shadow: 0 0 0 0 rgba(255, 154, 139, 0);
	}
}

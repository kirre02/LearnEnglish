<template>
  <div class="färger-page-container" @keydown="handleKeydown" tabindex="0">
    <div class="färger-container">

      <div class="färger-header" v-if="!isResultsMode">
      ...
      <div v-if="!isResultsMode">
        ...
        <div v-else-if="!quizFinished && currentQuestion" class="färger-content">
      ...
      <div v-else-if="isResultsMode" class="results-page">
        <h2 class="results-title">🎨 Dina quizresultat - Färger</h2>
...
export default {
  ...
  data() {
    ...
    const initialQuestions = [
      { question: "Vad betyder 'röd' på engelska?", options: shuffleArray(["red", "blue", "green", "yellow"]), correctAnswer: "red", hint: "Det är den färg man ofta kopplar till stoppskyltar", audioText: "red" },
      { question: "Vad betyder 'blå' på engelska?", options: shuffleArray(["blue", "red", "white", "black"]), correctAnswer: "blue", hint: "Färgen på himlen på en solig dag", audioText: "blue" },
      { question: "Vad betyder 'grön' på engelska?", options: shuffleArray(["green", "yellow", "blue", "red"]), correctAnswer: "green", hint: "Färgen på gräset", audioText: "green" },
      { question: "Vad betyder 'gul' på engelska?", options: shuffleArray(["yellow", "red", "blue", "gray"]), correctAnswer: "yellow", hint: "Färgen på solen", audioText: "yellow" },
      { question: "Vad betyder 'svart' på engelska?", options: shuffleArray(["black", "white", "gray", "red"]), correctAnswer: "black", hint: "Motsatsen till vit", audioText: "black" }
    ];
  ...
  computed: {
    ...
    resultsMessage() {
      const percentage = (this.score / this.questions.length) * 100;
      if (percentage >= 80) return 'Du är en riktig färgexpert!';
      if (percentage >= 60) return 'Du kan mycket om färger!';
      return 'Fortsätt öva, så blir du bättre på färger!';
    },
    isAnswerCorrect() {
      return this.selectedAnswer === this.currentQuestion.correctAnswer;
    },
  ...
  async mounted() {
    ...
    this.isResultsMode = this.$route.path.includes('/färger/results') || this.$route.query.showResults === 'true';
    
    try {
      if (this.isResultsMode) {
        console.log("📊 Laddar quizresultat för färger...");
        await this.loadPastResults();
      } else {
        console.log("🎨 Laddar färgfrågor...");
  ...
  try {
    ...
    useFallbackQuestions() {
      ...
      console.log('Använder fallback-frågor för färger');
    },
  ...
  watch: {
    '$route'(to) {
      if (to.path.includes('/färger/results')) {
        console.log("📡 Route changed → reload färger results");
        this.isResultsMode = true;
        this.loadPastResults();
...
<style scoped>
/* Färger-specifik styling med samma layout som Djur */
...
.play-again-btn {
	background: linear-gradient(135deg, #667eea, #764ba2); /* Färger tema-färg */
	color: white;
	border: none;
...
.results-table thead th {
	background-color: #667eea; /* Färger tema-färg */
	color: white;
	font-weight: bold;
...
.results-table tbody tr:hover {
	background-color: #f1f3ff; /* Lätt blå/lila hover för Färger-temat */
...
.back-from-results-btn {
	margin-top: 30px;
	background: linear-gradient(135deg, #667eea, #764ba2); /* Färger tema-färg (tillbaka-knapp) */
	color: white;
	padding: 12px 25px;
	border-radius: 25px;
	font-weight: bold;

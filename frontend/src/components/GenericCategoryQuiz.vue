<template>
  <div class="category-quiz-page-container" @keydown="handleKeydown" tabindex="0">
    <div class="category-quiz-container">
      <div class="category-quiz-header">
      ...
      <div v-if="!quizFinished" class="category-quiz-content">
        <div class="question-bubble">
          <div class="question-header">
            <div class="question-emoji">{{ categoryEmoji }}</div>
          </div>
...
export default {
  ...
  props: {
    ...
    categoryType: {
      type: String,
      required: true // 'färger', 'djur', 'familj', etc.
    }
  },
  data() {
    return {
  ...
  computed: {
    ...
    resultsMessage() {
      const percentage = (this.score / this.questions.length) * 100;
      if (percentage >= 80) return `Du är en riktig ${this.categoryName.toLowerCase()}-expert!`;
      if (percentage >= 60) return `Du kan mycket om ${this.categoryName.toLowerCase()}!`;
      return `Fortsätt öva på ${this.categoryName.toLowerCase()}, du blir bättre!`;
    },
    isAnswerCorrect() {
      return this.selectedAnswer === this.currentQuestion.correctAnswer;
    },
  ...
  mounted() {
    ...
    // Check if we should continue saved quiz
  ...
  methods: {
    ...
    // NY: Bekräfta avbrott
    confirmCancelQuiz() {
      if (confirm(`Vill du avbryta ${this.categoryName.toLowerCase()}? Ditt framsteg kommer att sparas så du kan fortsätta senare.`)) {
        this.cancelQuiz();
      }
    },
    ...
    cancelQuiz() {
      ...
      localStorage.setItem(`saved${this.categoryType.charAt(0).toUpperCase() + this.categoryType.slice(1)}State`, JSON.stringify(quizState));
      localStorage.removeItem(`current${this.categoryType.charAt(0).toUpperCase() + this.categoryType.slice(1)}State`);
    ...
    loadSavedQuiz() {
      const savedState = localStorage.getItem(`saved${this.categoryType.charAt(0).toUpperCase() + this.categoryType.slice(1)}State`);
      if (savedState) {
        try {
          ...
          console.log(`Fortsätter sparad ${this.categoryName} från fråga:`, this.currentQuestionIndex + 1);
          localStorage.removeItem(`saved${this.categoryType.charAt(0).toUpperCase() + this.categoryType.slice(1)}State`);
        } catch (e) {
          console.error(`Kunde inte ladda sparad ${this.categoryName}:`, e);
          this.loadCurrentQuizState();
        }
      } else {
        this.loadCurrentQuizState();
    ...
    saveCurrentQuizState() {
      ...
      localStorage.setItem(`current${this.categoryType.charAt(0).toUpperCase() + this.categoryType.slice(1)}State`, JSON.stringify(quizState));
    },

    loadCurrentQuizState() {
      const savedState = localStorage.getItem(`current${this.categoryType.charAt(0).toUpperCase() + this.categoryType.slice(1)}State`);
      if (savedState) {
        try {
          const state = JSON.parse(savedState);
          this.score = state.score || 0;
  ...
  beforeUnmount() {
    ...
    if (!this.quizFinished && !localStorage.getItem(`saved${this.categoryType.charAt(0).toUpperCase() + this.categoryType.slice(1)}State`)) { 
      this.saveCurrentQuizState();
    }
  }
}
...
<style scoped>
/* Använd samma CSS som din Quiz-komponent */
.category-quiz-page-container {
  min-height: 100vh;
...
/* Resten av CSS:en är identisk med din Quiz-komponent */
.back-btn {
